...
def cross_val_predict_dipls(Xs, Ys,Xt, lv,lamb, n_splits_cv = 10):
    
    
    lamb = np.array(np.repeat(lamb,lv)).astype(np.float64)
    cv_groups = np.repeat(np.arange(n_splits_cv), int(Xs.shape[0]/n_splits_cv)+1)[0:Xs.shape[0]]
    ypred_cv_dipls = np.zeros(Ys.shape)

    for ii in np.unique(cv_groups):

        Xs_test = Xs[cv_groups==ii,:]
        Ys_test = Ys[cv_groups==ii,:]
        Xs_cal = Xs[cv_groups!=ii,:]
        Ys_cal = Ys[cv_groups!=ii,:]



        model_dipls = dipals(Xs_cal-Xs_cal.mean(axis=0),Ys_cal-Ys_cal.mean(axis=0),
                        Xs_cal-Xs_cal.mean(axis=0),
                        Xt-Xt.mean(axis=0),
                        A = lv, 
                        l = lamb)

        B_dipls = model_dipls[0]
        beta_dipls_s = Ys.mean(axis=0) - (Xs.mean(axis=0).dot(B_dipls))


        Ys_test_pred = Xs_test.dot(B_dipls) + beta_dipls_s
        ypred_cv_dipls[cv_groups==ii,:] = Ys_test_pred.copy()

        
    return ypred_cv_dipls
